...
const CartForm = () => {
  ...
  const allChecked =
    cartItems.length > 0 &&
    cartItems.every((item) => checkedItems[item.id]);
    ...
    try {
      ...
      try {
        ...
        if (result.mergedCount > 0) {
          console.log(`중복된 장바구니 아이템 ${result.mergedCount}개를 정리했습니다.`);
        }
      } catch (mergeError) {
          ...
          <div className="font-bold text-sm">
            <div className="flex justify-between">
              <div>
                ...
                />
                <span>전체 선택 ({Object.values(checkedItems).filter(Boolean).length}/{cartItems.length})</span>
              </div>
